var/list/GMs=list("Dragonzues","Gitrekt","Ragnaroknite","Trainer1")
...
mob/proc/ChooseVisoredMask()
	if(!VisoredMask)
		switch(input("Choose Your Visored Mask","Visored Mask","Blank") in list("Horned","Dank","Blank"))
			if("Horned")
				usr.VisoredMask = "Horned"
				//usr.overlays+=usr.VisoredMask
			if("Dank")
				usr.VisoredMask = "Dank"
			//	usr.overlays+=usr.VisoredMask
			//	usr.AddHair(usr.HairStyle)
			else
				usr.VisoredMask = "Blank"
	else return
...
mob/GM/verb
	Announce(txt as text)
		set category="GM"
		world <<"<font color=white><font face=times new roman><font size=4><center>[usr] announces:<center><font color=green>[txt]"


	Smelt()
		set category="GM"
		var/Requirements=/obj/Items/EnemySpoils/Onyx
		var/Amt=5
		if(Requirements)
			for(var/obj/Items/I in usr.Inventory)
				if(istype(I,Requirements))
					if(I.CurStack>=Amt)
						alert("YAY")
	...
	ChooseVisored()
		set category="GM"
		if(!VisoredMask)
			switch(input("Choose Your Visored Mask","Visored Mask","Blank") in list("Horned","Dank"))
				if("Horned")
					usr.VisoredMask = "Horned"
				//usr.overlays+=usr.VisoredMask
				if("Dank")
		...
		if(VisoredMask)
			alert("You Already have a Mask Get a New one?","New Mask?","Yes","No")
			if("Yes")
				switch(input("Choose Your Visored Mask","Visored Mask","Blank") in list("Horned","Dank"))
					if("Horned")
						usr.VisoredMask = "Horned"
	...
	SetXPRate()
		set category="GM"
		if(!usr.key=="Dragonzues"||!usr.key=="Ragnaroknite"||!usr.key=="Gitrekt") alert("Your Not Allowed To Change This")
		else

			ServerExpRate =input(usr,"Choose Server's Xp Rate (Will Default to 1)","Server EXP Rate (Max 100)",ServerExpRate) as num
			if(ServerExpRate <= 1)
				ServerExpRate = 1
			//if(ServerExpRate >= 100)
			//	ServerExpRate = 100
/*
	SetEnemyIntensity()
		set category="GM"
		if(!usr.key=="Dragonzues"||!usr.key=="Ragnaroknite"||!usr.key=="Gitrekt") alert("Your Not Allowed To Change This")
			...
			EnemyIntensity =input(usr,"Choose Server's Intensity (Will Default to 1)","How Hard The Enemies Are",EnemyIntensity) as num
			if(EnemyIntensity <= 1)
				EnemyIntensity = 1
			if(EnemyIntensity >= 10)
				EnemyIntensity = 10
			for(var/mob/Enemy/E in world)
				E.Level *= EnemyIntensity
				E.LevelShift()
*/
	...
	Ban()
		...
		if(!M)	return
		if(M.key==world.host || M.key=="Dragonzues")
