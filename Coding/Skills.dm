	...
	Skills
		...
		Click(location,control,params)
			if(src.name=="Shikai")
				for(var/obj/Skills/SoulReaper/Shikai/S in usr.Skills)
					...
					if(usr.Zanpakuto)
						...
						if(Z.SpiritType=="Endless Flames")	usr.client.eye=locate(67,143,2)
						if(Z.SpiritType=="Kira")	usr.client.eye=locate(86,143,2)
						if(Z.SpiritType=="Rukia")	usr.client.eye=locate(105,143,2)

						usr.LoadSkillTree()

					return
			if(src.name=="Pet Skills")
				for(var/obj/Skills/Bount/Pet_Skills/B in usr.Skills)
					usr.client.eye=locate(48,143,2)
			if(src.name=="Bankai")
			...
			if(Learnable==2)
				...
				switch(ShowAlert(usr,"[src.name] - [src.SkillType] > > [src.desc] > > [LI][LD]",list("Level","Input","Close")))
					if("Level")
						if(usr.SkillPoints<=0)	return
						if(MS.Level<MS.MaxLevel)
					...
					if("Input")
						var/Amount =input("How Many To Add?","Level [src.name]") as num
						...
						if(Amount+MS.Level<=MS.MaxLevel)
							for(var/obj/Skills/S in usr.Skills)
								if(S.name==src.name)
									S.Level+=Amount
									if(S.Stat2Boost)
										usr.vars["[S.Stat2Boost]"]+=S.BoostAmount*Amount
										usr.HUDRefresh()
									break
		...
		Universal
			...
			Dodge
				...
				LevelDesc="+1.5% Chance per Level"
			...
			Spirit_Regen
				...
				LevelDesc="+30 Regen per Second per Level"


			Spirit_Charge
				icon_state="Spirit Charge";desc="Quickly Gather Spirit Energy to Restore Reiatsu > Also after Level 10 Start To Heal you > Hold to Charge"
				SkillType="Active"
			...
			ExoShell
				SkillType="Stat Booster"
				Stat2Boost="MaxSTM";BoostAmount=100
				icon_state="ExoShell";desc="Permanently Increases Max Stamina"
				LevelDesc="+100 Stamina per Skill Level"


			Spirit_Shine
				SkillType="Stat Booster"
				Stat2Boost="MaxREI";BoostAmount=100
				icon_state="Spirit Shine";desc="Permanently Increases Max Reiatsu"
				LevelDesc="+100 Reiatsu per Skill Level"


			Brute
				SkillType="Stat Booster"
				Stat2Boost="STR";BoostAmount=10
				icon_state="Brute";desc="Permanently Increases Strength"
				LevelDesc="+10 Strength per Skill Level"


			Stone_Skin
				SkillType="Stat Booster"
				Stat2Boost="VIT";BoostAmount=10
				icon_state="Stone Skin";desc="Permanently Increases Vitality"
				LevelDesc="+10 Vitality per Skill Level"


			Mystic_Glow
				SkillType="Stat Booster"
				Stat2Boost="MGC";BoostAmount=10
				icon_state="Mystic Glow";desc="Permanently Increases Magic"
				LevelDesc="+10 Magic per Skill Level"


			Mystic_Aura
				SkillType="Stat Booster"
				Stat2Boost="MGCDEF";BoostAmount=10
				icon_state="Mystic Aura";desc="Permanently Increases Magic Defense"
				LevelDesc="+10 Magic Defense per Skill Level"


			Blur
				SkillType="Stat Booster"
				Stat2Boost="AGI";BoostAmount=10
				icon_state="Blur";desc="Permanently Increases Agility"
				LevelDesc="+10 Agility per Skill Level"


			Lucky_Stars
				SkillType="Stat Booster"
				Stat2Boost="LCK";BoostAmount=10
				icon_state="Lucky Stars";desc="Permanently Increases Luck"
				LevelDesc="+10 Luck per Skill Level"


		Bount
			icon='ALLSkillIcons.dmi'
			...
			ShockWave
				SkillType="Attack"
				icon_state="ShockWave";desc="100+STR Damage to all Surrounding Enemies, 75 REI > Also every 10 Levels +1 range"
				LevelDesc="+25% Damage per Level"


			Spirit_Blast
				SkillType="Attack"
				icon_state="Spirit Blast";desc="Stun All Surrounding Enemies, 50 REI > Also every 10 Levels +1 range";
			...
			Brave_Shout
				SkillType="Active"
				icon_state="Brave Shout";desc="Taunt all nearby Enemies into Attacking you > Base Range of 4 > Also every 10 Levels +1 range"
		...
		SoulReaper
			...
			Understanding
				SkillType="Passive"
				MaxLevel=100
				icon_state="Understanding";desc="Increase to learn your Sword's Name"
			...
			Spirit_Shell
				...
				LevelDesc="+1.5% Decrease in Damage per Level"
			...
			Immunity
				...
				LevelDesc="+1.5% Immunity per Level"
			...
			Combat_Mastery
				...
				LevelDesc="+5% Damage per Attack per Level"
			...
			Shikai_Duration
				...
				icon_state="Shikai Duration";desc="Time Limit your allowed in Shikai"
				LevelDesc="Requires Shikai"
			...
			Bankai
				...
				LevelDesc="Requires Special Training available at Level 150"

			Bankai_Duration
				SkillType="Passive"
				Level=1;MaxLevel=200
				icon_state="Bankai Duration";desc="How Long you can Stay in Bankai"
				LevelDesc="Requires Bankai"

			Visored
				SkillType="Active"
				Level=1;MaxLevel=1
				icon_state="Visored";desc="Unite with your Inner Hollow"
				LevelDesc="Requires a Test available at Level 400"

			Visored_Duration
				SkillType="Passive"
				Level=1;MaxLevel=200
				icon_state="Visored Duration";desc="How Long you can Become one with your Inner Hollow"
				LevelDesc="Requires Visored"
		...
		Quincy
			...
			Energy_Expert
				...
				LevelDesc="+1.5 Creation Speed Boost per Level"
			...
			Marksman
				...
				LevelDesc="+1.5 Charge Speed Boost per Level"
		...
		Shikais
			...
			Light_Dragon
				...
				Petal_Attack
					SkillType="Attack"
					icon_state="Petal Attack";desc="Fire a Ball of Blade Petals > Consumes 10 Rei"
					LevelDesc="Damage = 25 + MGC. > Plus 1 Base Damage per Level > +25% Damage per Level"
			...
			Endless_Flames
				...
				Flame_Ring
					SkillType="Active"
					icon_state="Flame Ring";desc="Burn the ground surrounding the player for 3 seconds. > Anyone inside the area during activation will be Burned > Consumes 350 Rei"
					LevelDesc="Leveling Increases Burn Radius and Duration"


				Fire_Dragon_Assault
					SkillType="Attack"
					icon_state="Fire Dragon Assault";desc="Launch an Elemental Dragon from your Sword. Burn Opponents. > Damage = 250+MGC > Consumes 200 Rei"
					LevelDesc="+10% Damage per Level"


				Fire_Blade
					SkillType="Passive"
					icon_state="Fire Blade";desc="Attacks have a chance of Burning the Target. > Only Active during Shikai"
					LevelDesc="+10% Chance to Burn per Level"
			...
			Kira
				...
				Heavy_Weight
					SkillType="Passive"
					icon_state="Heavy Weight";desc="Decrease Movement Speed of Enemies"
					LevelDesc="Increases Effect by 1 for every 10 levels"

				Maximum_Pressure
					SkillType="Active"
					icon_state="Maximum_Pressure";desc="Those Around you become Slow"
					LevelDesc="Increases Effect by 1 for every 10 levels"


			Rukia
				icon='ShikaiRukia.dmi'

				Ice_Barrage
					SkillType="Attack"
					icon_state="Ice Barrage";desc="Launch a Barrage of Ice Crystals. > Damage = 250+MGC > Consumes 200 Rei"
					LevelDesc="+10% Damage per Level"

				Ice_Queen
					SkillType="Active"
					icon_state="Ice Queen";desc="Deal Damage in an AoE around you , when in Bankai those that get too close become Frozen"
					LevelDesc="+10% Damage and +1 Range per 10 Levels"
		...
		Hollow
			...
			Ressurection
				MaxLevel=1
				icon_state="Ressurection";desc="Use a Zanpaktou To increase your Fighting Prowness"


			Ressurection_Duration
				MaxLevel=5000
				icon_state="Ressurection Duration";desc="How Long us can use your Release"
